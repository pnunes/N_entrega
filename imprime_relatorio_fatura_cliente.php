...
<head>
   ...
   <!-- <link href="css/bootstrap.min.css" rel="stylesheet" id="bootstrap-css">-->
	...
	<link rel="stylesheet" type="text/css" href="css/estilo_imprime_fatura_ciente.css">
	<?php
...
<body>
   <?php
       ...
       switch (get_post_action('cliente')) {
		  case 'cliente':
		      $cliente      = $_POST['s_cliente']; 
			  //colocando as data de pesquisa numa variavel global.
			  $_SESSION['cliente_m'] = $cliente;
			  $sql_cli = "SELECT nome FROM pessoa WHERE cnpj_cpf='$cliente'";
			  $resulcli = mysqli_query($cone,$sql_cli) or die ("Não foi possivel acessar o banco");
			  $tota_cli = mysqli_num_rows($resulcli);
			  for($cc=0;$cc < $tota_cli; $cc++) {
				 $row_cli = mysqli_fetch_row($resulcli);
				 $nome_cli = $row_cli[0];
			  }
			  ...
			  //echo "<p>Cliente :".$_SESSION['cliente_m'];
			  header("Location:gera_relatorios_faturamento.php?tipo_rela=fatu_cliente");
   ...
   <form name="cadastro" id="cadastro" align="center" action="" method="post">
        <table id="tb_cliente">
		   ...
		   <tr>
		      <td id="cliente">
				...
				<div class="col-md-10" id="dv_cliente">
					<select id="s_cliente" name="s_cliente" class="form-control">
        ...
        <div style="width:1150px;height:50px;background-color:#191970;vertical-align:middle !important;z-index:1;position:relative;margin-left:100px;margin-top:0px;">
		   ...
		   <div style="width:85px;height:40px;float:left;position:relative;margin-left:7px;margin-top:6px;">
			   <button type="Submit" id="bt_cliente" name="cliente" class="btn btn-primary">Gerar Relatorio</button>
		   </div
		</div>
