...
<head>
   ...
   <!-- <link href="css/bootstrap.min.css" rel="stylesheet" id="bootstrap-css">-->
	...
	<link rel="stylesheet" type="text/css" href="css/estilo_imprime_fatura_escritorio.css">
	<?php
...
<body>
   <?php
       ...
       switch (get_post_action('escritorio')) {
		  case 'escritorio':
		 
		      $escritorio   = $_POST['s_escritorio']; 
			  
			  //colocando as data de pesquisa numa variavel global.
			  $_SESSION['escritorio_m'] = $escritorio;
			  
			  $sql_escri = "SELECT nome FROM escritorio WHERE codigo='$escritorio'";
			  $res_cri = mysqli_query($cone,$sql_escri) or die ("Não foi possivel acessar o banco");
			  $tota_es = mysqli_num_rows($res_cri);
			  
			  for($ce=0; $ce < $tota_es; $ce++) {
				 $row_es = mysqli_fetch_row($res_cri);
				 $nome_escri = $row_es[0];
			  }
			  $_SESSION['nome_esc_m'] = $nome_escri;
			  ...
			  header("Location:gera_relatorios_faturamento.php?tipo_rela=fatu_escritorio");
   ...
   <form name="cadastro" id="cadastro" align="center" action="" method="post">
        <table id="tb_cliente">
		   ...
		   <tr>
		      <td id="cliente">
				...
				<div class="col-md-10" id="dv_escritorio">
					<select id="s_escritorio" name="s_escritorio" class="form-control">
						<option value="">Selecione o Escritório</option>
						<?php
						$sql2 = "SELECT codigo,nome FROM escritorio";
        ...
        <div style="width:1150px;height:50px;background-color:#191970;vertical-align:middle !important;z-index:1;position:relative;margin-left:100px;margin-top:0px;">
		   ...
		   <div style="width:85px;height:40px;float:left;position:relative;margin-left:7px;margin-top:6px;">
			   <button type="Submit" id="bt_escritorio" name="escritorio" class="btn btn-primary">Gerar Relatorio</button>
		   </div
		</div>
