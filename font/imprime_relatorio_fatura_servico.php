...
<head>
   ...
   <!-- <link href="css/bootstrap.min.css" rel="stylesheet" id="bootstrap-css">-->
	...
	<link rel="stylesheet" type="text/css" href="css/estilo_imprime_fatura_servico.css">
	<?php
...
<body>
   <?php
       ...
       switch (get_post_action('servico')) {
		  case 'servico':
		 
		      $s_servico   = $_POST['s_servico']; 
			 
			  //colocando as data de pesquisa numa variavel global.
			  $_SESSION['s_servico_m'] = $s_servico;
			  
			  $sql_servi = "SELECT descri_se FROM servico WHERE codigo_se='$s_servico'";
			  $res_ser = mysqli_query($cone,$sql_servi) or die ("Não foi possivel acessar o banco");
			  $tota_ser = mysqli_num_rows($res_ser);
			  			  
			  for($se=0; $se < $tota_ser; $se++) {
				 $row_ser = mysqli_fetch_row($res_ser);
				 $descri_servi = $row_ser[0];
			  }
			  $_SESSION['descri_serve_m'] = $descri_servi;
			  ...
			  //vai para rorina que imprime relatorio
			  header("Location:gera_relatorios_faturamento.php?tipo_rela=fatu_servico");
   ...
   <form name="cadastro" id="cadastro" align="center" action="" method="post">
        <table id="tb_cliente">
		   ...
		   <tr>
		      <td id="cliente">
				<?php if(isset($_SESSION['servico_m'])) {
					$servico = $_SESSION['servico_m'];
				} else {
					$servico ='';
				}?>
				<div class="col-md-10" id="dv_servico">
					<select id="s_servico" name="s_servico" class="form-control">
						<option value="">Selecione o Serviço</option>
						<?php
						$sql2s = "SELECT codigo_se,descri_se FROM servico";
						$resuls = mysqli_query($cone,$sql2s) or die ("Não foi possivel acessar o banco");
						while ( $linha = mysqli_fetch_array($resuls)) {
							$select = $servico == $linha[0] ? "selected" : "";
        ...
        <div style="width:1150px;height:50px;background-color:#191970;vertical-align:middle !important;z-index:1;position:relative;margin-left:100px;margin-top:0px;">
		   ...
		   <div style="width:85px;height:40px;float:left;position:relative;margin-left:7px;margin-top:6px;">
			   <button type="Submit" id="bt_servico" name="servico" class="btn btn-primary">Gerar Relatorio</button>
		   </div
		</div>
